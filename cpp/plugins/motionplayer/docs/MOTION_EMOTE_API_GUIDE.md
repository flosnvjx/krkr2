# MotionPlayer / EmotePlayer 技术文档与使用指南
...
> **实现计划与验收：** [`../PLAN.md`](../PLAN.md)  
> **progress 行为对照：** [`progress-flow-comparison.md`](progress-flow-comparison.md)
...
**C++ 内部 `isEmoteLikeMotion()`（非脚本 API）：** 当 `type==1` 或 `variableLabels` 非空时为 true，用于 **Player 运行时** 调整 eval / `updateLayers` 实现分支（见 [`progress-flow-comparison.md`](progress-flow-comparison.md) §2）。**不是** sdl3 的 progress 双路径，也 **不是** E-mote 能否运行的权威判定。
...
**sdl3 参考：** 仅有一个 `EmotePlayer::progress`，内部用 `_varList.size()>0` 决定传给树的 `tick`（0 vs `clockPassed`）；**无** KrKr2 式 `progressEmoteLike` / `frameProgress` 函数对。详见 [`progress-flow-comparison.md`](progress-flow-comparison.md) §0 勘误。
...
| 历史文档中的「卡死 = 子 Player 全量 updateLayers」 | **已作废**（未证实）；见 `progress-flow-comparison.md` §0 |
...
| [`../PLAN.md`](../PLAN.md) | KrKr2 替代实现计划、NEKOPARA 验收 |
| [`progress-flow-comparison.md`](progress-flow-comparison.md) | progress 对照与 **2026-05-28 勘误** |
| [`motionplayer-render-test-feasibility.md`](motionplayer-render-test-feasibility.md) | 渲染测试方案 |
| [`../sdl3/README.md`](../reverse_engineering/README.md) | sdl3 只读参考说明 |
| `data/system/AffineSourceMotion.tjs` | 生产脚本调用链 |
| `docs/AGENTS.md` §6.2 | Agent 导航 |
...
- **2026-05-28 勘误：** 撤回「必须 progress 双路径 / 卡死=子 Player」；见 [`progress-flow-comparison.md`](progress-flow-comparison.md) §0。

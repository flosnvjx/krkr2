...
namespace emoteplayer {
    emotemotion::emotemotion(emotefile *filePtr, uint32_t startOffset) :
        _filePtr(filePtr) {
        // dic
        ...
        for(size_t i = 0; i < _layer.size(); i++) {
            emotenode *tmp =
                new emotenode(this, nullptr, nodeList, filePtr, _layer.at(i));
            layer.push_back(tmp);
        }
        ...
        for(auto node_idx = priority.begin(); node_idx != priority.end();
            ++node_idx) {
    ...
    emotenode *emotemotion::getNodeByName(const std::string &name) {
        for(auto node : nodeList) {
            if(node == nullptr)
                continue;
            if(strcmp(name.c_str(), node->label.c_str()) ==
               0) // 仅靠一个label，合理吗？
            {
                return node;
            }
    ...
    void emotemotion::progress(float tick, std::vector<emoteRender> &renderList,
                               emotelimit lim) {
        // 树状便利，构建节点的渲染结构
        for(auto ch : layer) {
            renderMethod.clear();
    ...
    void emotemotion::drawToLayer(tTJSNI_BaseLayer *layer, emotelimit lim,
                                  tTJSNI_BaseLayer *maskLayer) {
        std::vector<emotenode *> result, resultEx;
        if(_filePtr->isMotion) {
            for(auto node_idx = nodeList.rbegin(); node_idx != nodeList.rend();
                ++node_idx) {
            ...
            std::stable_sort(
                result.begin(), result.end(), [](emotenode *a, emotenode *b) {
    ...
    void emotemotion::drawToBitmap(iTVPBaseBitmap *target,
    ...
    emoteobject::emoteobject(emotefile *filePtr, uint32_t startOffset) :
        _filePtr(filePtr) {
        // dic
        std::map<std::string, uint32_t> _rootData;
        filePtr->parseObject(_rootData, startOffset);
        ...
        for(auto _obj : _motion) {
            ...
            motion.insert(
                std::pair<std::string, emotemotion *>(_obj.first, tmp));
        }
    }

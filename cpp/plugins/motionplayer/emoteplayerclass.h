...
namespace emoteplayer {
    enum MaskMode { MaskModeNone, MaskModeAlpha };

    enum PlayFlag { PlayFlagNone, PlayFlagForce };
...
#define property_marco(name, type, storage)                                    \
    ...
    public:
        ...
        EmotePlayer(ResourceManager *resourceManager) :
        ...
        void setDrawAffineTranslateMatrix(tjs_real a, tjs_real b, tjs_real c,
                                          tjs_real d, tjs_int tx, tjs_int ty);
        ...
        void setTimelineBlendRatio(tTJSString name, tjs_real ratio,
                                   tjs_real time = 0, tjs_real easing = 0);
        void fadeInTimeline(tTJSString name, tjs_real time = 0,
                            tjs_real easing = 0);
        void fadeOutTimeline(tTJSString name, tjs_real time = 0,
                             tjs_real easing = 0);
    ...
    private:
        ...
        bool isSelfClear = false; // true:draw使用完全copy(以实现自主clear)
                                  // false:通过clear函数间接完成
                                  // 其分别对应了两种emote的运行模式
        // data
        ...
        void drawWithTVP(tTJSNI_BaseLayer *layer);

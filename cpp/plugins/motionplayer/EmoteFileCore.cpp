...
namespace emoteplayer {
    ...
    emotefile::~emotefile() { ClearAniTree(); }

    void emotefile::setSeed(tjs_int seed) { _psbFile.setSeed(seed); }
    ...
    bool emotefile::parseList(std::vector<uint32_t> &output,
                              uint32_t objOffset) {
    ...
    uint64_t emotefile::GetSize() { return _psbFile.getDataSize(); }
    ...
    bool emotefile::GenerateAniTree() {
        ...
        for(auto _obj : _tmpMap) {
            ...
            _objects.insert(
                std::pair<std::string, emoteobject *>(_obj.first, tmp));
        }
        // screen
        ...
        for(auto _obj : _tmpMap) {
            ...
            _source.insert(
                std::pair<std::string, emotesource *>(_obj.first, tmp));
        }
        // stereovisionProfile
        ...
        for(auto _objTmp : _objects) {
            for(auto _tmpMtn : _objTmp.second->motion) {
                std::vector<emotenode *> _stack(
                    _tmpMtn.second->nodeList.begin(),
                ...
                while(!_stack.empty()) {
                    ...
                    for(auto frameTmp : _current->frameList) {
                        if(frameTmp != nullptr && frameTmp->hasContent) {
                            ...
                            emotemotion *_emot =
                                findmotionByName(frameTmp->src);
                            if(_emot != nullptr) {
                                for(auto it = _emot->nodeList.begin();
                                    it != _emot->nodeList.end(); ++it) {
                                    _stack.push_back(*it);
                                }
                            }
                        }
        ...
        for(auto attrItm : _metadata->_attrcomp) {
            ...
            for(auto removeDataItm : attrItm->data_remove) {
                ...
                if(cha != _objects.end()) {
                    auto mtn =
                        cha->second->motion.find(removeDataItm.motion.c_str());
                    if(mtn != cha->second->motion.end()) {
                        auto lay =
                            mtn->second->getNodeByName(removeDataItm.layer);
                        if(lay != nullptr) {
                            lay->removed = true;
                        }
                    }
    ...
    float emotefile::getSyncTime() { return _syncTime; }
    ...
    bool emotefile::readIconTobuffer(uint8_t *buff, uint32_t buffSize,
                                     uint32_t pitch, emoteicon *ic) {
        const auto &chunkLengths = _psbFile.getChunkLengths().value;
        if(ic->pixel < 0 ||
           ic->pixel >= static_cast<int32_t>(chunkLengths.size()))
            return false;
        // 生数据读取
        uint8_t *srcbuff = new uint8_t[chunkLengths.at(ic->pixel)];
        if(!_psbFile.readChunkData(ic->pixel, srcbuff,
                                   chunkLengths.at(ic->pixel))) {
            delete[] srcbuff;
            return false;
        }
        // 解码方案
        if(strcmp(ic->compress.c_str(), "RL") == 0) {
            if(ic->pal > -1) {
                ...
                if(!_psbFile.readChunkData(ic->pal, palbuff,
                                           chunkLengths.at(ic->pal))) {
                    delete[] srcbuff;
                    delete[] palbuff;
                    return false;
                }
                ...
                while(currPtr < endPtr && currSize < buffSize) {
                    ...
                    } else // not redundant
                    {
                        ...
                        for(int i = 0; i < count; i++) {
                            memcpy(currDst + i * 4, palbuff + (*(currPtr + i)),
                                   4);
                        }
                        currPtr += count;
                        currDst += count * 4;
            ...
            if(!isKrkr) // 采用裁切贴图
            {
                if(strcmp(ic->type.c_str(), "RGBA8") == 0) {
                    uint8_t *start =
                        srcbuff + (int)(ic->top * ic->texWidth + ic->left) * 4;
                    uint32_t cppitch =
                        std::min((uint32_t)(std::min(ic->left + ic->width,
                                                     ic->texWidth) -
                                            ic->left) *
                                     4,
                                 pitch);
                    uint32_t height =
                        std::min((uint32_t)(std::min(ic->top + ic->height,
                                                     ic->texHeight) -
                                            ic->top),
                                 buffSize / pitch);
                    for(uint32_t i = 0; i < height; i++) {
                        memcpy(buff + i * pitch,
                               start + (int)(ic->texWidth * 4 * i), cppitch);
                    }
                } else {
        ...
        for(auto itm : _metadata->_eyeControl) {
            ...
            if(itm->isBlinking) {
                if(tick - itm->lastTick > itm->blinkFrameCount) // 是否结束
                {
                    ...
                    if((tick - itm->lastTick) * 2 <
                       itm->blinkFrameCount) // 闭眼
                    {
                        realVal += (tick - itm->lastTick) * 2 *
                            (itm->endFrame - itm->beginFrame) /
                            itm->blinkFrameCount;
                    } else // 睁开
                    {
                        realVal +=
                            (itm->blinkFrameCount - (tick - itm->lastTick)) *
                            2 * (itm->endFrame - itm->beginFrame) /
                            itm->blinkFrameCount;
                    }
    ...
    void emotefile::startTimeline(float tick, const std::string &name,
                                  bool isMain) {
        // 附属
        if(isMain) {
            for(auto itm : _attach) {
    ...
    void emotefile::stopTimeline(const std::string &name, bool isMain) {
        ...
        for(auto itm : _metadata->_timelineControl) {
            if(strcmp(itm->label.c_str(), name.c_str()) == 0) {
                currTimeline.erase(
                    std::remove(currTimeline.begin(), currTimeline.end(), itm),
                    currTimeline.end());
                currStartTick = -1.0f;
                return;
            }
    ...
    bool emotefile::checkTimline(const std::string &name, bool &result,
                                 bool isMain) {
        // 附属
        if(isMain) {
            for(auto itm : _attach) {
    ...
    void emotefile::updateTimelineControl(float tick, bool isMain) {
        ...
        for(auto timelineItm : currTimeline) {
            ...
            if(timelineItm->loopEnd > 0.0f &&
               tick - currStartTick + timelineItm->loopBegin >
                   timelineItm->loopEnd) {
                // 重置时间戳
                currStartTick = tick;
            }
            ...
            for(auto varItm : timelineItm->variableList) {
                ...
                for(auto sleItm : _metadata->_selectorControl) {
                    for(auto _sleItm : sleItm->selectItem) {
                        if(strcmp(_sleItm.label.c_str(),
                                  varItm->label.c_str()) == 0) {
                            isfindInSelect = true;
                            break;
                        }
                    }
                ...
                if(currFrameIdx >= 0 &&
                   currFrameIdx < varItm->frameList.size() - 1)
                     ...
                     nextframe->type == 2))) // 存在下一帧则对关键帧进行插值
                {
                    ...
                    realVal = (currFrame->value +
                               (nextframe->value - currFrame->value) /
    ...
    void emotefile::updatePhysics(float tick) {
        ...
        // }
        //  刮风
        hair_sim.startWind(glm::vec2(0.0f, 0.0f), glm::vec2(1280.0f, 0.0f),
                           0.15, 0.2, 0.3);
        for(auto itm : _metadata->_hairControl) {
            ...
            // 获取飘动值
            float hair_sway =
                hair_sim.generate(tick, glm::vec2(640.0f, 0.0f), -30.f, 30.f);
    ...
    void emotefile::addEmoteFile(emotefile *itm) { _attach.push_back(itm); }
} // namespace emoteplayer

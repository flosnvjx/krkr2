...
namespace emoteplayer {
    emotenode::emotenode(emotemotion *rootmotion, emotenode *parent,
                         std::vector<emotenode *> &nodeList, emotefile *filePtr,
                         uint32_t startOffset) :
        _parent(parent), _rootmotion(rootmotion), _filePtr(filePtr) {
        // add to mgn ss
        ...
        if(it != _rootData.end())
            filePtr->parseList(_tmpList, it->second);
        for(size_t i = 0; i < _tmpList.size(); i++) {
            emotenode *tmp = new emotenode(_rootmotion, this, nodeList, filePtr,
                                           _tmpList.at(i));
    ...
    iTVPBaseBitmap *emotenode::getSourceBitmap() const {
        ...
        if(_cachedSourceBitmap == nullptr || _cachedSourceWidth != width ||
           _cachedSourceHeight != height) {
            if(_cachedSourceBitmap != nullptr)
                delete _cachedSourceBitmap;
            _cachedSourceBitmap =
                new tTVPBaseTexture(static_cast<tjs_uint>(width),
        ...
        if(_cachedSourceIcon != ic) {
            if(_filePtr->colorType == 0) {
                ...
                _cachedSourceBitmap->Update(rgba.data(), width * 4, 0, 0, width,
                                            height);
            } else {
                _cachedSourceBitmap->Update(data, width * 4, 0, 0, width,
                                            height);
            }
    ...
    void emotenode::drawPatchCommon(const emotelimit &lim,
                                    iTVPBaseBitmap *sourceBitmap,
                                    tTJSNI_BaseLayer *layer,
                                    iTVPBaseBitmap *target,
        ...
        std::vector<glm::mat4> patchTransforms(
            static_cast<size_t>(surfaceCount));
        std::vector<const float *> patchControlPoints(
            static_cast<size_t>(surfaceCount));
        int idxCnt = 0;
        ...
        for(int32_t i = surfaceCount - 1; i >= 0; i--) {
            ...
            patchTransforms[static_cast<size_t>(idxCnt)] =
                renderMethod.at(i).matTrans;
            patchControlPoints[static_cast<size_t>(idxCnt)] =
                renderMethod.at(i).type == 1 ? renderMethod.at(i).controlPts
        ...
        if(layer != nullptr) {
            emoteDrawPatchToLayer(
                layer, lim, idxCnt, glm::value_ptr(patchTransforms[0]),
                patchControlPoints.data(), sourceBitmap, width, height,
                totalOpa, currbm, meshDivision, useBezierMesh);
        } else if(target != nullptr) {
            emoteDrawPatchToBitmap(target, targetClip, lim, idxCnt,
                                   glm::value_ptr(patchTransforms[0]),
                                   patchControlPoints.data(), sourceBitmap,
                                   width, height, totalOpa, currbm,
                                   meshDivision, useBezierMesh);
        }
    }
    void emotenode::checkDrawStatus(float tick,
                                    std::vector<emoteRender> &renderList,
                                    emotelimit lim) {
        // 不绘制进行节点传递
        ...
        if(tmpic != nullptr) {
            ...
            if(tmpic != ic) // 直接比对ic
            {
                ...
                originY = ic->originY;
                _cachedSourceIcon = nullptr;
                if(!_filePtr->readIconTobuffer(data, width * height * 4,
                                               width * 4, ic)) {
            ...
            // 设置混色
            currbm = frame->bm;
        } else if(emot != nullptr ||
                  strcmp(frame->src.c_str(), "layout") == 0 ||
    ...
    void emotenode::progress(float tick, std::vector<emoteRender> &renderList,
                             emotelimit lim) {
        // 参数化时可能改变
        currTick = tick;
        ...
        // 再来一个非时间戳节点, 采用时间永驻机制
        if(_filePtr->isMotion && frameList.size() == 2 &&
           frameList.at(0)->type == 2 && frameList.at(1)->type == 0)
        ...
        if((!isNeedDraw && (width == 0 || height == 0)) || type == 7) {
                 ...
                 nextframe->type == 2))) // 存在下一帧则对关键帧进行插值
            {
                ...
                currCoordx = (frame->coordX +
                              (nextframe->coordX - frame->coordX) /
                                  (nextframe->time - frame->time) *
                                  (currTick - frame->time));
                currCoordy = (frame->coordY +
                              (nextframe->coordY - frame->coordY) /
                                  (nextframe->time - frame->time) *
                                  (currTick - frame->time));
                currCoordz = (frame->coordZ +
                              (nextframe->coordZ - frame->coordZ) /
                                  (nextframe->time - frame->time) *
                                  (currTick - frame->time));
                // 透明度
                currOpa = (frame->opa +
                           (nextframe->opa - frame->opa) /
                               (nextframe->time - frame->time) *
                               (currTick - frame->time));
                   ...
                   frame->angle > 180) // 从 小360 到大0
                {
                    currAngle = (frame->angle - 360 +
                                 (nextframe->angle + 360 - frame->angle) /
                                     (nextframe->time - frame->time) *
                                     (currTick - frame->time));
                } else if(nextframe->angle > 180 &&
                          frame->angle < 180) // 从 大0 到 小360
                {
                    ...
                    currAngle = (frame->angle +
                                 (nextframe->angle - frame->angle) /
                                     (nextframe->time - frame->time) *
                                     (currTick - frame->time));
                }
                currSx = (frame->sx +
                          (nextframe->sx - frame->sx) /
                              (nextframe->time - frame->time) *
                              (currTick - frame->time));
                currSy = (frame->sy +
                          (nextframe->sy - frame->sy) /
                              (nextframe->time - frame->time) *
                              (currTick - frame->time));
                currZx = (frame->zx +
                          (nextframe->zx - frame->zx) /
                              (nextframe->time - frame->time) *
                              (currTick - frame->time));
                currZy = (frame->zy +
                          (nextframe->zy - frame->zy) /
                              (nextframe->time - frame->time) *
                              (currTick - frame->time));
                currOx = (frame->ox +
                          (nextframe->ox - frame->ox) /
                              (nextframe->time - frame->time) *
                              (currTick - frame->time));
                currOy = (frame->oy +
                          (nextframe->oy - frame->oy) /
                              (nextframe->time - frame->time) *
                              (currTick - frame->time));
                // 偏移
                currTimeOffset = (frame->timeOffset +
                                  (nextframe->timeOffset - frame->timeOffset) /
                                      (nextframe->time - frame->time) *
                                      (currTick - frame->time));
                // 网格参数
            ...
            if(renderMethod.size() > 0 && currCoordz != 0.0) {
                if(renderMethod.at(0).type == 3) {
                    renderMethod.at(0).matTrans =
                        glm::translate(renderMethod.at(0).matTrans,
                                       glm::vec3(0.0, 0.0, currCoordz));
                } else {
                    glm::mat4 proj = glm::ortho(
                        -renderMethod.at(0).originX,
                    ...
                    last =
                        glm::translate(last, glm::vec3(0.0, 0.0, currCoordz));
                    last =
                        last * glm::inverse(proj) * renderMethod.at(0).matTrans;
            ...
            // 构建变换矩阵 平移 currCoordx/currCoordy → 缩放 zx/zy → 剪切 sx/sy
            // → 旋转 angle
            ...
            model =
                glm::mat4(1.0f, currSy, 0.0f, 0.0f, currSx, 1.0f, 0.0f, 0.0f,
            ...
            if(isLayout) // 不绘制，不处理网格数据，只构建变换矩阵
            {
                if(renderMethod.size() > 0 &&
                   renderMethod.back().type ==
                       3) // 如果上一级是layout的话，则进行合并
                {
                    // 构造渲染方法结构
                    emoteRender emt = renderMethod.back();
                    renderMethod.pop_back();
                    ...
                    if(renderMethod.size() > 1 &&
                       renderMethod.back().type == 2) {
                        glm::mat4 demuxMat = glm::scale(
                            renderMethod.back().matTrans,
                            glm::vec3(1 / renderMethod.back().width,
                                      1 / renderMethod.back().height, 1.0f));
                        demuxMat = glm::translate(
                            demuxMat,
                            glm::vec3(renderMethod.back().originX,
                        ...
                        emt.matTrans =
                            glm::inverse(glm::ortho(-originX, width - originX,
                                                    height - originY, -originY,
                                                    lim.zMax, -lim.zMax)) *
                            emt.matTrans;
                    } else // 正常更新矩阵
                    {
                       ...
                       3) // 如果上一级是layout的话，则进行合并
                {
                    ...
                    glm::mat4 projection =
                        glm::ortho(-lim.originX, lim.width - lim.originX,
                                   lim.height - lim.originY, -lim.originY,
                                   lim.zMax, -lim.zMax);
                    model = glm::translate(
                        model,
                    ...
                    if(renderMethod.size() > 0 &&
                       renderMethod.back().type == 2) {
                        ...
                        model =
                            glm::translate(model,
                                           glm::vec3(-originX - currOx,
                                                     -originY - currOy, 0.0f));
                        model =
                            glm::scale(model, glm::vec3(width, height, 1.0f));
                        emt.matTrans = demuxMat * model;
                        emt.opa *= renderMethod.back().opa;
                        renderMethod.pop_back();
                    } else // 正常更新矩阵
    ...
    void emotenode::drawToLayer(tTJSNI_BaseLayer *layer, emotelimit lim,
    ...
    void emotenode::drawToBitmap(iTVPBaseBitmap *target,
                                 const tTVPRect &targetClip, emotelimit lim,
                                 tTJSNI_BaseLayer *maskLayer) {
        if(emot != nullptr) {
